...
def access_text(repo_url: str) -> str:
    return (
        ...
        "Как прогнать свое КП в чате с Claude\n"
        "1. Откройте claude.ai и новый чат.\n"
        "2. Вставьте ссылку на инструкцию и попросите прочитать ее целиком, без пересказа: "
        + raw_url(repo_url, "prompt.md") + "\n"
        "3. Следом вставьте текст вашего КП и ответы на шесть вопросов: клиент, проект, "
        "кто получит КП, стадия сделки, цель клиента, бюджет.\n"
        "4. Claude вернет файл отчета. Скачайте его и откройте в браузере на компьютере: "
        "файл сам подтянет шаблон из репозитория и соберет отчет.\n"
        "5. Кнопка «Сохранить HTML» внизу даст автономную копию, ее можно отправлять кому угодно.\n"
        "6. Если вместо файла пришел JSON, скачайте из репозитория goodkp.html, откройте, "
        "вставьте JSON и нажмите «Собрать отчет».\n\n"
        "Как прогнать через агента\n"
        "Claude Code или Cursor: дайте ссылку на репозиторий " + repo_url + ", "
        "текст КП и шесть ответов, скажите «прогони по prompt.md и собери report.html». "
        "Агент заполнит данные, проверит их и отдаст файл.\n\n"
        "Как прогнать из терминала\n"
        "Команда uv run goodkp run kp.pdf --answers answers.yaml в работе, "
