...
def access_text(repo_url: str) -> str:
    return (
        ...
        "Это скилл для Claude.\n"
        "В Claude Code: git clone " + repo_url + " ~/.claude/skills/good-kp, "
        "дальше в любом проекте команда /good-kp и файл КП.\n"
        "В claude.ai: настройки, раздел Skills, загрузите репозиторий архивом "
        "(кнопка Code, Download ZIP на странице репозитория). В чате напишите «прогони КП» "
        "и приложите файл.\n"
        "Claude задаст шесть вопросов: клиент, проект, кто получит КП, стадия сделки, "
        "цель клиента, бюджет. Потом соберет отчет и отдаст HTML-файл.\n\n"
        "Методика\n"
